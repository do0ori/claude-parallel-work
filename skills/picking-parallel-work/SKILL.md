...
### 1-1. 끝난 워크트리가 있으면 먼저 치운다

조사 결과에 `Worktree ... is finished` 경고가 있으면 **묻지 말고 정리하라.**

```
node "<이 스킬 디렉터리>/scripts/cleanup.mjs" --apply
```

무엇을 지웠는지는 보고하되, 지워도 되냐고 확인받지 마라. 안전장치는 사람의
확인이 아니라 스크립트의 검사에 있다 — locked 워크트리, 커밋이 기본 브랜치에
다 안 들어간 워크트리, 변경사항이 남은 워크트리는 스크립트가 알아서 건너뛰고
무엇을 왜 남겼는지 적는다. 브랜치 삭제도 `-d` 라서 머지되지 않았으면 git 이
거절한다.

지우는 것은 **이 플러그인이 만든 워크트리와 그 브랜치뿐**이다. 저장소의 다른
머지된 브랜치는 건드리지 않는다 — 그건 이 도구의 일이 아니다.

무엇을 지울지만 보고 싶으면 `--apply` 를 빼면 되지만, 고르기 흐름에서는 쓰지
마라. 사람을 한 번 더 세우는 값이 없다.
...
**Priority·Status 는 라벨로도 들 수 있다.** Project 에 값이 없으면 같은 이름의
라벨을 찾는다 — `priorityOrder`·`statusOrder`·`excludeStatuses` 에 적힌 값이
그대로 라벨 이름이 된다. **보드 값이 있으면 보드가 이긴다**(라벨은 폴백이다).
비교는 영역 라벨과 같은 규칙이라 `p0` 이 `P0` 과, `in-progress` 가
`In Progress` 와 맞는다.

보드를 아예 못 쓰는 저장소가 있어서 필요하다 — 저장소와 보드의 소유자가 달라
연결되지 않거나, 외부 협업자가 섞여 보드 권한을 나눌 수 없거나(조직 기본
권한은 멤버에게만 적용된다). 보드가 없다고 순위와 제외가 함께 죽으면 이 도구의
본래 일이 안 된다.

**하드 제외**(후보에서 아예 빠짐)는 이런 경우다.

- **다른 사람이** 담당이다 (내가 담당인 것은 빼지 않는다 — 1 순위로 올라온다)
- Status 가 제외 목록에 있다 (보드 값 또는 같은 이름의 라벨)
...
쪼개진 이슈의 **부모는 작업 단위가 아니다.** 열린 하위 이슈가 있으면 부모를
빼고 그 하위 이슈를 후보로 둔다. 이슈 번호는 부모가 항상 작아서, 막지 않으면
부모가 자기 자식보다 먼저 제안된다.
...
열리는 창은 **별도의 창**이다. 사람이 못 찾을 수 있으니 그 사실을 함께 전하라.
...
작업이 끝나면 **먼저 기본 브랜치 위로 최신화한다.**

```
git pull --rebase origin <기본 브랜치>
```

병렬 작업에서 이건 선택이 아니다. 브랜치를 딴 뒤 기본 브랜치는 다른 세션들이
머지하면서 계속 움직인다. 최신화하지 않으면 **CI 가 낡은 기반에서 통과한
결과로 머지된다** — 각자 통과했지만 합치면 깨지는 조합이 정확히 이 구도에서
나온다. 리베이스 후에는 강제 푸시가 필요하고(`git push --force-with-lease`),
CI 를 다시 통과하는 것까지 확인해야 머지 준비가 끝난다.

충돌이 나면 **혼자 풀지 말고 멈춰서 알린다.** 다른 세션이 같은 자리를 이미
바꿨다는 뜻이고, 그때는 어느 쪽이 맞는지가 사람의 판단이다.
...
**머지 자체는 사람이 결정한다.** 초록이 됐다고 머지하지 마라 — 리뷰가 남아
있을 수 있고, 여러 브랜치를 굴리는 중이면 머지 순서가 판단 대상이다.

**머지된 뒤에는 Status 를 Done 으로 내린다.**

```
node "<이 스킬 디렉터리>/scripts/set-status.mjs" <번호> Done
```

이걸 빼먹으면 보드가 조용히 낡는다. 그리고 그냥 지저분해지는 정도가 아니다 —
`In Progress` 는 하드 제외 대상이라, 머지가 끝났는데 그대로 남은 이슈는 후보로
영영 돌아오지 않는다. 부분만 끝낸 이슈라면 그 뒷부분을 아무도 집지 못한다.

### 7. 워크트리를 치운다

**이 작업 안에서 끝내라.** 다음 작업에 떠넘기지 않는다.

원격 브랜치는 여기서 바로 지울 수 있다. 주 체크아웃을 건드리지 않기 때문이다.

```
git push origin --delete <영역>/<슬러그>
```

GitHub 이 머지할 때 이미 지웠으면 실패하는데, 무해하다.

워크트리 자체는 `ExitWorktree` 도구를 `action: "remove"` 로 호출해 지운다.
워크트리에서 빠져나오면서 디렉터리와 브랜치를 함께 없앤다.

그 도구가 "활성 워크트리 세션이 없다" 고 하면 이 세션은 그 방법으로 지울 수
없다. 그때는 세션을 끝내라 — Claude Code 가 종료할 때 keep 인지 remove 인지
묻고, remove 를 고르면 같은 결과가 된다.

둘 다 못 했으면 워크트리가 남는다. 그래도 막다른 길은 아니다. 다음에 주
체크아웃에서 작업을 고를 때 `cleanup.mjs` 가 안전망으로 치운다. 다만 그건
안전망일 뿐이고, 치우는 것은 이 작업의 일이다.
