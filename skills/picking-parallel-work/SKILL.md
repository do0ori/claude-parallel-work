...
조사 결과에 `Worktree ... is finished` 경고가 있으면 고르기 전에 정리하라.

```
node "<이 스킬 디렉터리>/scripts/cleanup.mjs"          무엇을 지울지 본다
node "<이 스킬 디렉터리>/scripts/cleanup.mjs" --apply  지운다
```

끝난 워크트리를 남겨두면 그 영역을 계속 점유해서 판단을 망친다. 지우는 것은
**이 플러그인이 만든 워크트리와 그 브랜치뿐**이다. 저장소의 다른 머지된 브랜치는
건드리지 않는다 — 그건 이 도구의 일이 아니다.

locked 워크트리, 커밋이 남은 워크트리, 변경사항이 있는 워크트리는 건너뛴다.
무엇을 왜 남겼는지 출력에 적힌다.
