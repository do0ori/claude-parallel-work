...
**하드 제외**(후보에서 아예 빠짐)는 **다른 사람이** 담당이거나, Status 가 제외
목록에 있거나, 진행 중인 워크트리·PR 이 그 이슈를 참조하는 경우다. 내가 담당인
것은 빼지 않는다 — 1 순위로 올라온다.
...
빠진 것이 있으면 survey.mjs 가 **고르는 단계에서 미리** 경고로 알려준다 — 새 세션이
빌드부터 실패하고 원인을 찾아 헤매게 두지 않는다.
